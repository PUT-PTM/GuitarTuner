...
q31_t Input_q31[SAMPLES];
double Output[FFT_SIZE]; /*!< Output buffer is always FFT_SIZE */
double Output_Mag[FFT_SIZE];
double Output_Mag_f32[FFT_SIZE];/*!< Output buffer is always FFT_SIZE */
q31_t Output_Mag_q31[FFT_SIZE];
...
void UART_send(int * arr, uint32_t size)
{
	for(int i=0;i<size;i++)
	{
		char display[8]= {0};
...
/*
		arm_cfft_radix4_instance_q31 S_q31;

		for(int i=0;i<SAMPLES;i++)
		{
			Input[i] = ((int)Input[i] << 19);
		}

		 arm_cfft_radix4_init_q31(&S_q31, FFT_SIZE, 0,1);

		 arm_float_to_q31(Input, Input_q31, SAMPLES);

		 arm_cfft_radix4_q31(&S_q31, Input_q31);

		 arm_cmplx_mag_squared_q31(Input_q31, Output_Mag_q31, FFT_SIZE);
		 Output_Mag_q31[0]=0;

		 arm_max_q31(Output_Mag_q31, FFT_SIZE, &MaxValue, &MaxIndex);
*/
